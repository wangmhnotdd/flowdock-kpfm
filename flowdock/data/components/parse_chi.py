...
def get_dihedral_indices(onehot_sequence):
    return np.array(
        [dihedral_indices[aa_idx2aa_short[aa_idx]] for aa_idx in np.where(onehot_sequence)[1]]
    )
...
def test_get_chi_angles(print_chi_angles=False):
    ...
    log.info("test_get_chi_angles passed")
    try:
        os.remove("6w70.pdb.gz")
    except Exception as e:
        log.warning(f"Error removing file: {e}")
    if print_chi_angles:
        log.info(chi_angles)
    return True
