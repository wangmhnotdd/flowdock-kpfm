...
VDW_RADII_MAPPING = {row["atomic_number"]: row["vdw_radius"] for _, row in ptable.iterrows()}
...
def get_vdw_radii_array():
    ...
    return np.array(
        [VDW_RADII_MAPPING[i] if i in VDW_RADII_MAPPING.keys() else -1.0 for i in range(119)]
    )


def get_vdw_radii_array_uff():
    """Get UFF van der Waals radii array."""
    return np.array(
        [
...
def calc_heavy_atom_LJ_clash_fraction(mol1, mol2, LJ_CUTOFF=100):
    """Calculate the fraction of heavy atoms in mol1 that have a Lennard-Jones interaction energy
    greater than LJ_CUTOFF with any atom in mol2."""
    ...
    pairwise_LJ = D_ij * ((x_ij / distance_matrix) ** 12 - 2 * (x_ij / distance_matrix) ** 6)
    return np.sum(np.sum(pairwise_LJ, axis=1) > LJ_CUTOFF) / pairwise_LJ.shape[0]


def calc_heavy_atom_LJ_int_energy(mol1, mol2):
    """Calculate the Lennard-Jones interaction energy between two molecules."""
    D_ij, x_ij = get_epsilon_sigma_uff(mol1, mol2)
    coords1 = np.array(mol1.GetConformer().GetPositions())
    coords2 = np.array(mol2.GetConformer().GetPositions())
    distance_matrix = np.linalg.norm(
...
def get_epsilon_sigma_uff(m1, m2):
    """Get epsilon and sigma for Lorentz-Berthelot mixing rule."""
    Di = [float(uff_params_df.loc[atom.GetSymbol(), "energy"]) for atom in m1.GetAtoms()]
    Dj = [float(uff_params_df.loc[atom.GetSymbol(), "energy"]) for atom in m2.GetAtoms()]
    xi = [float(uff_params_df.loc[atom.GetSymbol(), "distance"]) for atom in m1.GetAtoms()]
    xj = [float(uff_params_df.loc[atom.GetSymbol(), "distance"]) for atom in m2.GetAtoms()]
