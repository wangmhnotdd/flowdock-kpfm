...
class FlowDock(torch.nn.Module):
    ...
    def reverse_interp_vdode_step(
        ...
        """Reverse process sampling using an Euler Variance Diminishing ODE (VD-ODE) solver.
    ...
    def sample_pl_complex_structures(
        ...
        **kwargs,
        ...
        with torch.no_grad():
            ...
            # NOTE: We follow https://arxiv.org/pdf/2402.04845.pdf for symbolic conventions
            # NOTE: In the FlowDock paper, `t=0` and `t=1` are the start and end times, respectively,
            # yet here we reverse the notation such that `t=1` and `t=0` are the start and end times, correspondingly,
            # to simplify the integration of this implementation with the original NeuralPLexer code
